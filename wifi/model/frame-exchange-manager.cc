...
FrameExchangeManager::GetAckManager (void) const
{
 ...
 uint_least16_t FrameExchangeManager::getLinkId() {
  return m_linkId;
 }
int FrameExchangeManager::var=0;
...
//Time nn= Simulator::Now ();
//std::cout<<"FEM Rx"<<nn.GetMilliSeconds()<<std::endl;


 //int t=-2,tt=-2;
 // t=m_linkId;

  //tt=m_phy->GetChannelNumber();
 //std::cout<<"FEM in link "<<t<<"  Phy is  "<<tt<<std::endl;
...
int FrameExchangeManager::count1=0;
int FrameExchangeManager::count2=0;
bool
FrameExchangeManager::StartTransmission (Ptr<Txop> dcf, uint16_t allowedWidth)
{
  // Time nn= Simulator::Now ();
//std::cout<<"FEM Tx"<<nn.GetMilliSeconds()<<std::endl;
  
  //tt=m_phy->GetChannelNumber();
 // std::cout<<"START TX FEM in link "<<t<<"  Phy is  "<<tt<<std::endl;
...
// else
// count2++;
//std::cout<<"link 0 count "<<count1<<"   link 1 count "<<count2<<"   ";
//m_linkId=1;----------------------------
  ...
  //int s=-2;
  if(mpdu->GetPacketSize()>600)
  std::cout<<"I am "<<mpdu->GetPacketSize()<<std::endl;
  ...
  //     int t=-1;
  // t=m_linkId;
  // std::cout<<"FEM link "<<t<<"   seq#  "<<sequence<<"  size is "<<mpdu->GetPacketSize()<<"\n";
      //std::cout<<"FEM Seq# "<<sequence;
     // std::cout<<"FEM sequeuence number "<<sequence<<std::endl;
...
//std::cout<<mpdu->GetTimeStamp()<<std::endl;
  SendMpduWithProtection (mpdu, txParams);
//int t=-2;
      //t=m_linkId;
     // std::cout<<"  LinkID is "<<t<<std::endl;
  return true;
}
...
{ int tt=-2;
  tt=m_linkId;
  //std::cout<<"FEM link ID"<<tt<<std::endl;
...
//   tt=m_phy->GetChannelNumber();
  //std::cout<<"FEM in link "<<t<<"  Phy is  "<<tt<<" size of Mpdu  "<<std::endl;
  if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::NONE)
    {
...
FrameExchangeManager::ForwardMpduDown (Ptr<WifiMacQueueItem> mpdu, WifiTxVector& txVector)
{
  ...
  // tt=m_phy->GetChannelNumber();
  // std::cout<<"FEM in link "<<t<<"  Phy is  "<<tt<<" size of Mpdu  "<<mpdu->GetSize()<<std::endl;

  m_phy->Send (Create<WifiPsdu> (mpdu, false), txVector);
 // std::cout<<"I am FEM "<<m_phy->GetChannelNumber();
}
                               ...
                               WifiTxVector txVector, std::vector<bool> perMpduStatus)
{
  ...
  // t=m_linkId;
  // tt=m_phy->GetChannelNumber();
 // std::cout<<"FEM linkId "<<t<<" channel number "<<tt<<std::endl;
