...
ChannelAccessManager::SetupFrameExchangeManager (Ptr<FrameExchangeManager> feManager)
{
  ...
  //std::cout<<"CAM phy"<<m_linkId<<std::endl;
}
...
// ch=m_phy->GetChannelNumber();
// std::cout<<"CAM I am the channe number "<<ch<<std::endl;
  NS_LOG_FUNCTION (this << txop);
  if (m_phy)
    {
  ...
  DoGrantDcfAccess ();
  //std::cout<<"CAMMMMMMMMMMMMMMMMMMMM\n";
...
ChannelAccessManager::DoGrantDcfAccess (void)
{
  ...
  Time now = Simulator::Now ();
   for (Txops::iterator i = m_txops.begin (); i != m_txops.end (); k++)
    {
      //std::cout<<"m_linkId "<<m_linkId<<std::endl;
...
ChannelAccessManager::GetAccessGrantStart (bool ignoreNav) const
{
  ...
  // std::cout<<"CAM   "<<t<<"   Address of pointer  "<<std::endl;
  NS_LOG_FUNCTION (this);
  ...
  for (auto txop : m_txops)
    {
      ...
      //           int t=-2,tt=-2;
      //  t=backoffUpdateBound.GetMilliSeconds();
      //  tt=m_linkId;
      //  std::cout<<"CAM ---  "<<t<<"link ID "<<tt<<std::endl;
...
ChannelAccessManager::NotifyTxStartNow (Time duration)
{
  //  int tt=-2;
  // tt=m_linkId;
  // std::cout<<"CAM link ID"<<tt<<std::endl;
  NS_LOG_FUNCTION (this << duration);
  m_lastRxReceivedOk = true;
  Time now = Simulator::Now ();
  ...
  if (m_lastRx.end > now)
    {
     // std::cout<<"channel access manager I am\n ";
