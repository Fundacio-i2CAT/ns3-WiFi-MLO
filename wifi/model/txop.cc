...
Txop::Txop (Ptr<WifiMacQueue> queue)
  : m_queue (queue)
{
    //m_queue1(CreateObject<WifiMacQueue> (AC_BE))
...
// bb=linkId;
  NS_LOG_FUNCTION (this);
  auto& link = GetLink (linkId);
  //std::cout<<"TXOP I am called for link "<<bb<<std::endl;
...
Txop::UpdateBackoffSlotsNow (uint32_t nSlots, Time backoffUpdateBound, uint8_t linkId)
{
  ...
  double Txop::AvgBO=0;
  uint32_t Txop::MinBO=99999;
  uint32_t Txop::MaxBO=0;
  int Txop::countfile=0;
void
Txop::StartBackoffNow (uint32_t nSlots, uint8_t linkId)
{
  ...
  std::ofstream write;
 write.open("/home/suneel/Desktop/Scenario/LogBackoff.txt", std::ios::app);  
 if(nSlots>=MaxBO)
      MaxBO=nSlots;
  if(nSlots<=MinBO)
      MinBO=nSlots;
  ...
  { int temp=linkId;
    if(countfile++==32)
    write<<"Tnow\t\tBO now\t\tAvgBO\t\t\tMAxBO\t\tlink\n";
    write<<Simulator::Now ().GetMilliSeconds()<<"\t\t"<<nSlots<<"\t\t"<<(int)AvgBO<<"\t\t\t"<<MaxBO<<"\t\t"<<temp<<"\n";
     AvgBO=AvgBO*.8+.2*nSlots;
...
int n=0;//hdr.getlinkId();

//change 1 added new code
if(GetNLinks()>=2)
{
...
if(GetNLinks()<2)//normal case
{
   
  n=0;
}

if(n==0&&m_queue->getlink1()==500||n==1&&m_queue->getlink2()==500);
else
{
  if (m_mac->GetChannelAccessManager (n)->NeedBackoffUponAccess (this)){
      GenerateBackoff (n);
     } 

  m_queue->Enqueue (Create<WifiMacQueueItem> (packet, hdr));
  
    StartAccessIfNeeded (n);  
...
{     
  ...
  if (HasFramesToTransmit () && GetLink (linkId).access == NOT_REQUESTED)
    {
          //change 2 in code
          Ptr<const WifiMacQueueItem> Pk= m_queue->Peek();
          if(GetNLinks()>=2){

            // if((Pk->GetPacketSize()==386&&linkId==1)||(Pk->GetPacketSize()<386)||(Pk->GetPacketSize()==736&&linkId==0))
