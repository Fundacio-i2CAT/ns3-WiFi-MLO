...
float num_pktNoSLA;
  ...
  float PbSTA1_link1;
  float PbSTA1_link2;
  float SLA_ViolationError;
  double Avg_pktDelay_STA1_link1;
  double Avg_pktDelay_STA1_link2;
  double Avg_pktDelay_STA1;
  double TT_current;
  int count_ReTx_l1;
  int count_ReTx_l2;
  static int flag_Ext_CSLAMLO;//1 st change

void read(double &STA1_CurrentDelay_link1,double &STA1_CurrentDelay_link2,double &STA1_CurrentDelay,int flow,float D);

void Scheduler(float D,float T ,int flow);
static const int TotalFLows=6;

static bool FlagArray[TotalFLows];

bool CheckFlag(int flow);

static int Totalcount_Extour;
