...
QosFrameExchangeManager::CancelPifsRecovery (void)
{
 ...
 int QosFrameExchangeManager::var=0;
bool
...
//   m_linkId=1;
   ...
   //std::cout<<"QOS frame link  "<<t<<"   Time "<<n<<std::endl;
...
//   //here
 // std::cout<<"QOS FEM  I am in StartTX    "<<tt<<std::endl;
...
QosFrameExchangeManager::StartFrameExchange (Ptr<QosTxop> edca, Time availableTime, bool initialFrame)
{
  ...
  Ptr<const WifiMacQueueItem> mpdu = edca->PPeekNextMpdu (m_linkId);
                                     ...
                                     Time availableTime) const
{
  //sequence numbers
  //  int n=1;
  //  n=   mpdu->GetHeader().GetSequenceNumber();
   
  // std::cout<<"QOSFEM "<<n<<"    "<<mpdu->GetPacketSize()<<std::endl;
  NS_ASSERT (mpdu);
