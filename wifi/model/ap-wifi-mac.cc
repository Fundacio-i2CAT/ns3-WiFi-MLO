...
//setting the background traffic
  if(hdr.GetAddr1().ConvertIntoInt()>=2&&hdr.GetAddr1().ConvertIntoInt()<=6)
    hdr.SetlinkId(0);
  else if(hdr.GetAddr1().ConvertIntoInt()>=7&&hdr.GetAddr1().ConvertIntoInt()<=12)
    hdr.SetlinkId(Iam_link);
  else if(hdr.GetAddr1().ConvertIntoInt()>=13)
    hdr.SetlinkId(1);
  else
  {
 ...
 //  if(var)
    ...
    read(STA1_CurrentDelay_link1,STA1_CurrentDelay_link2,STA1_CurrentDelay,1,1.2);//reading current delay from file
    if(Simulator::Now ().GetSeconds()<2.5)
       {
         Avg_pktDelay_STA1_link1=var;
         Avg_pktDelay_STA1_link2=!var;

       }
    else
       {
         Avg_pktDelay_STA1_link1=0.8*Avg_pktDelay_STA1_link1+STA1_CurrentDelay_link1*0.2;
         Avg_pktDelay_STA1_link2=0.8*Avg_pktDelay_STA1_link2+STA1_CurrentDelay_link2*0.2;

       }
       
       // std::cout<<PbSTA1_link1<<std::endl;
  ...
  if (GetQosSupported ())
    {
      //std::cout<<"AP wifiMAC frwd down "<<packet->GetSize()<<"\t TID"<<t<<std::endl;
...
{
  //std::cout<<"AP wifiMAC frwd down "<<packet->GetSize()<<std::endl;
    Time now = Simulator::Now ();
    //std::cout<<"AP Wifi MAC Time "<<now.GetMicroSeconds()<<std::endl;
  //std::cout<<"AP wifiMAC  "<<packet->GetSize();
...
ApWifiMac::TxOk (Ptr<const WifiMacQueueItem> mpdu)
{
  ...
  // if(mpdu->GetSize()>250)
  //     std::cout<<"\nAP End to End delay   "<<tt<<"\t Size:  "<<mpdu->GetSize()<<"\n";
...
ApWifiMac::Receive (Ptr<WifiMacQueueItem> mpdu, uint8_t linkId)
{
  ...
  //std::cout<<"APwifi I am working\t"<<<<std::endl;
