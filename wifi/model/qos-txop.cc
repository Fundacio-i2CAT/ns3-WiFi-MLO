...
QosTxop::IsQosOldPacket (Ptr<const WifiMacQueueItem> mpdu)
{
 ...
 int QosTxop::count1=0;
 int QosTxop::count2=0;
Ptr<const WifiMacQueueItem>
QosTxop::PPeekNextMpdu (uint8_t linkId,uint8_t tid, Mac48Address recipient, Ptr<const WifiMacQueueItem> item)
{
  ...
  auto peek = [this, &tid, &recipient, &item,&linkId] () -> Ptr<const WifiMacQueueItem>
    {
      if (tid == 8 && recipient.IsBroadcast ())  // undefined TID and recipient
        {  
        // change 3 added in code 
          if(GetNLinks()>=2)
            return  m_queue-> SearchIITEM(linkId,m_qosBlockedDestinations, item);
          else // normal case
            return m_queue->PeekFirstAvailable (m_qosBlockedDestinations, item);;
        }
      if (m_qosBlockedDestinations->IsBlocked (recipient, tid))
        {
           //std::cout<<"\nI am QOTXOP Blocked"<<count2++;
  ...
  std::ofstream WriteSTA1,WriteSTA2,WriteAP,Packet_count;

  if(item)
  {
    WriteAP.open("FreshScenarios/QueueAP.txt", std::ios::app);
    WriteSTA1.open("FreshScenarios/QueueSTA1.txt", std::ios::app);
    WriteSTA2.open("FreshScenarios/QueueSTA26.txt", std::ios::app);
    int link1=0,link2=0;
    m_queue-> Count_packet_inQUEUE();


    if(item->GetHeader().GetAddr1().ConvertIntoInt()==200)
        WriteAP<<Simulator::Now ().GetMicroSeconds()<<","<<m_queue->GetCurrentSize().GetValue()
          <<","<<m_queue->getlink1()<<","<<m_queue->getlink2()<<std::endl;
    if(item->GetHeader().GetAddr2().ConvertIntoInt()==1||item->GetHeader().GetAddr1().ConvertIntoInt()==1)
        WriteSTA1<<Simulator::Now ().GetMicroSeconds()<<","<<m_queue->GetCurrentSize().GetValue()
         <<","<<m_queue->getlink1()<<","<<m_queue->getlink2()<<std::endl;
    if(item->GetHeader().GetAddr2().ConvertIntoInt()==26||item->GetHeader().GetAddr1().ConvertIntoInt()==26)
        WriteSTA2<<Simulator::Now ().GetMicroSeconds()<<","<<m_queue->GetCurrentSize().GetValue()
         <<","<<m_queue->getlink1()<<","<<m_queue->getlink2()<<std::endl;
      ...
      if (tid == 8 && recipient.IsBroadcast ())  // undefined TID and recipient
        {
         ...
         //Ptr<const WifiMacQueueItem> PKK= m_queue->PeekFirstAvailable (m_qosBlockedDestinations, item);
         //std::cout<<"IN QOSTXOP ..........link "<<item-GetPacketSize()<<std::endl;
          return m_queue->PeekFirstAvailable (m_qosBlockedDestinations, item);;
        }
      if (m_qosBlockedDestinations->IsBlocked (recipient, tid))
        {
...
int link=-1;
int ii=peekedItem->GetPacketSize();
if(ii==386)
link =1;
else 
link =0;
  ...
  auto qosFem = StaticCast<QosFrameExchangeManager> (m_mac->GetFrameExchangeManager (link));
...
// ii=peekedItem->GetPacketSize();

//   std::cout<<"\n size is "<<ii<<"  link is "<<qosFem->getLinkId();
...
QosTxop::AddBaResponseTimeout (Mac48Address recipient, uint8_t tid)
{
 ...
 //change 4 add in code
int n=0;

int t=m_queue->Peek()->GetPacketSize();
// std::cout<<t<<std::endl;
// if(t>=700)
// n=0;
// else if(t>=300)
  ...
  if (m_baManager->ExistsAgreementInState (recipient, tid, OriginatorBlockAckAgreement::PENDING))
    {
      ...
      GenerateBackoff (n);
      if (HasFramesToTransmit () && GetLink (n).access == NOT_REQUESTED)
        {
          m_mac->GetChannelAccessManager (n)->RequestAccess (this);
        }
    }
    //nn=tid;
 // std::cout<<"QOS TXOP "<<nn<<<<std::endl;
}
