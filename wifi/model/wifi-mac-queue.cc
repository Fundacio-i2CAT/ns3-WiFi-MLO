    ...
    NS_LOG_TEMPLATE_DEFINE ("WifiMacQueue")
{
  ...
  void WifiMacQueue::Inclink1(){link1++;}
  int WifiMacQueue::getlink1(){ return link1;}
  void WifiMacQueue::Declink1(){ link1--;}
  void WifiMacQueue::Inclink2(){ link2++;}
   void WifiMacQueue::Declink2(){ link2--;}
  int WifiMacQueue::getlink2(){ return link2;}
...
//custome function
 Ptr<const WifiMacQueueItem> WifiMacQueue::Peek (u_int8_t linkID) const 
{
  NS_LOG_FUNCTION (this);
  const Time now = Simulator::Now ();
  for (auto it = GetContainer ().cbegin (); it != GetContainer ().cend (); it++)
    {
      ...
      if (now <= (*it)->GetTimeStamp () + m_maxDelay)
        {
          if(DoPeek (it)->GetHeader().getlinkId()==linkID)
               return DoPeek (it);
        }
    }
  NS_LOG_DEBUG ("The queue is empty");
...
//counts number of 350/700 packets 
void
WifiMacQueue::Count_packet_inQUEUE() {
    ConstIterator it = GetContainer ().cbegin ();
   link1=0;
   link2=0;
    while (it != GetContainer ().cend ())
    {
    if ((*it)->GetHeader().getlinkId()==0)
      link1++;
    else
      link2++;
...
Ptr<const WifiMacQueueItem>
WifiMacQueue::SearchIITEM(int linkId,const Ptr<QosBlockedDestinations> blockedPackets,
                                  Ptr<const WifiMacQueueItem> item) const
{
  NS_LOG_FUNCTION (this << item);
  NS_ASSERT ( !item || item->IsQueued ());
  ...
  while (it != GetContainer ().cend ())
    {
      // if((*it)->GetPacketSize()==386&&linkId==0)
      // std::cout<<"\n386 is going through 0 link";
      // skip packets that stayed in the queue for too long. They will be
      // actually removed from the queue by the next call to a non-const method
    //  std::cout<<(*it)->GetHeader().getlinkId()<<" - "<<linkId<<"  -  "<<(*it)->GetPacketSize()<<std::endl;
      int flag=(*it)->GetHeader().getlinkId();
      bool cntrl=(*it)->GetHeader().IsData();
    
       if ((flag==1&&linkId==1)||(flag==0&&linkId==0)||!cntrl){
          //if (((*it)->GetPacketSize()==386&&linkId==1)||((*it)->GetPacketSize()==736&&linkId==0)||(*it)->GetPacketSize()<200)
     // if ((linkId==1)||(linkId==0)||(*it)->GetPacketSize()<200)
        //if(((*it)->GetPacketSize()==100&&((flag==1&&linkId==1)||(flag==0&&linkId==0)))||(*it)->GetPacketSize()!=100)
        {
          
         // std::cout<<"\nMAC QUEUEU link  "<<linkId<<"  Packet size  "<<(*it)->GetPacketSize();
          if (!(*it)->GetHeader ().IsQosData () || !blockedPackets
              || !blockedPackets->IsBlocked ((*it)->GetHeader ().GetAddr1 (), (*it)->GetHeader ().GetQosTid ()))
            {
              return *it;
