...
// double StaWifiMac::TCurrent=0;
// double StaWifiMac::TCurrent_LLC=0;
...
StaWifiMac::Enqueue (Ptr<Packet> packet, Mac48Address to)
{
 ...
 int no_pkt_link=0;
int STANum =hdr.GetAddr2().ConvertIntoInt();


  //Setting Background on STANum 1
  //STA16 to STA22
  if(STANum>=16 && STANum<=22)//  
  {
    
    hdr.SetlinkId(0);
  }
  //Setting Background on link 2
  //STA12 to STA 15
   else if(STANum>=12 && STANum<=15)
  {
      
    hdr.SetlinkId(1);
    
  }
  //MLD STA1 to STA11 with different delay bound and error threshold
  else if (STANum>=1 && STANum<=11)
  {
    ...
    if(STANum==1)
    {
      read_file.open("FreshScenarios/STA1_Avg_Values.txt", std::ios::app);
      D=120;
      flow=1;
      Error=.05;//in %
    Scheduler(D,Error,flow); //1
    }     
    else if (STANum==2)
    {
      read_file.open("FreshScenarios/STA2_Avg_Values.txt", std::ios::app);
        D=50000;
      flow=2;
      Error=.10;//in %
      Scheduler(D,Error,flow); //2
    }
    else if (STANum==3)
      {
        
        read_file.open("FreshScenarios/STA3_Avg_Values.txt", std::ios::app);
       D=50000;
      flow=3;
      Error=.1;//in %
     Scheduler(D,Error,flow); //3
      }
      
    
    else if (STANum==4)
    {
       read_file.open("FreshScenarios/STA4_Avg_Values.txt", std::ios::app);
     D=50000;
      flow=4;
      Error=.1;//in %
     Scheduler(D,Error,flow); //4

    }
    else if (STANum==5)
    {
       read_file.open("FreshScenarios/STA5_Avg_Values.txt", std::ios::app);
        D=50000;
      flow=5;
      Error=.10;//in %
     Scheduler(D,Error,flow); //5

    }
    else if (STANum==6)
    {
       read_file.open("FreshScenarios/STA6_Avg_Values.txt", std::ios::app);
        D=200;
      flow=6;
      Error=.1;//in %
    Scheduler(D,Error,flow); //6

    }
     else if (STANum==7)
    {
       read_file.open("FreshScenarios/STA7_Avg_Values.txt", std::ios::app);
       D=50000;
      flow=7;
      Error=.1;//in %
    Scheduler(D,Error,flow); //6
    ...
    {
       read_file.open("FreshScenarios/STA8_Avg_Values.txt", std::ios::app);
        D=50000;
      flow=8;
      Error=.1;//in %
    Scheduler(D,Error,flow); //6
    ...
    {
       read_file.open("FreshScenarios/STA9_Avg_Values.txt", std::ios::app);
        D=50000;
      flow=9;
    Error=.10;//in %
    Scheduler(D,Error,flow); //6
    ...
    {
       read_file.open("FreshScenarios/STA10_Avg_Values.txt", std::ios::app);
       D=1000;
      flow=10;
      Error=.1;//in %
    Scheduler(D,Error,flow); //6
      ...
      else if (hdr.GetAddr2().ConvertIntoInt()==11)
    {
       read_file.open("FreshScenarios/STA11_Avg_Values.txt", std::ios::app);
       D=50000;
      flow=11;
      Error=.10;//in %
    Scheduler(D,Error,flow); //6
   ...
   var=!var;
   int STA_number=hdr.GetAddr2().ConvertIntoInt();
   if(Simulator::Now ().GetSeconds()<=2.5)
    ...
    read_file<<Simulator::Now ().GetMicroSeconds()<<","<<Avg_pktDelay_STA1_link1<<","<<Avg_pktDelay_STA1_link2<<std::endl;
    read_file.close();
    if(Simulator::Now ().GetMilliSeconds()-TCurrent*1000>250/2&&1)
     {
          
          
              TCurrent=Simulator::Now ().GetSeconds();
...
//Based on probabaliry and rand_num, assign link 1 or link 2
    if(rand_num<PbSTA1_link1)
       {         
          hdr.SetlinkId(0);
          flag1=true;
        }
...
StaWifiMac::Receive (Ptr<WifiMacQueueItem> mpdu, uint8_t linkId)
{
  ...
  // int tt=-2;
//  int b=-1;
//  b= mpdu->GetHeader().GetSequenceNumber();
                                    ...
                                    Mac48Address bssid, uint8_t linkId)
{
  ...
  if (GetQosSupported ())
    {
      ...
      const EdcaParameterSet& edcaParameters = beacon.GetEdcaParameterSet ();
  ...
  if (GetVhtSupported ())
    {
      const VhtCapabilities& vhtCapabilities = beacon.GetVhtCapabilities ();
  ...
  if (GetHtSupported ())
    {
      // const ExtendedCapabilities& extendedCapabilities = beacon.GetExtendedCapabilities ();
  ...
  if (GetHeSupported ())
    {
      ...
      const MuEdcaParameterSet& muEdcaParameters = beacon.GetMuEdcaParameterSet ();
  ...
  if (GetEhtSupported ())
    {
      const EhtCapabilities& ehtCapabilities = beacon.GetEhtCapabilities ();
