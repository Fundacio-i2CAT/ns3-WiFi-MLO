 ...
 * Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USAAvg_pktDelay_STA1_link2;
 *
...
//defined new variables
  ...
  PbSTA1_link1=0.5;
  PbSTA1_link2=0.5;
  SLA_ViolationError=0;
  Avg_pktDelay_STA1_link1=0.0000;
  Avg_pktDelay_STA1=0.0;
  Avg_pktDelay_STA1_link2=0.0000;
  TT_current=0;
  count_ReTx_l1=0;
  count_ReTx_l2=0;
...
int WifiMac::flag_Ext_CSLAMLO =-1;
...
bool WifiMac::FlagArray[TotalFLows]={0,0,0,0,0,0};

void CountRetranmission(int flow, int &countlink1, int &countlink2)
{
  // int countlink1=0, countlink2=0;
  std::ifstream read;
  read.open("FreshScenarios/LogRetransmission.txt");
  if(read){
     while(!read.eof()) 
    {
      int ReTx, link;
      double a,time_;
      char b;
      read>>time_>>b>>a>>b>>ReTx>>b>>a>>b>>link;
      if(ReTx==flow)
      {
        if(link==0)
          countlink1++;
...
bool WifiMac::CheckFlag(int flow)
{
 ...
 int WifiMac::Totalcount_Extour=0;

void WifiMac::Scheduler(float D,float T,int flow){

  std::ofstream scheduler_read, tem_read;
  


  if(flow==1)
  scheduler_read.open("FreshScenarios/Scheduler1.txt", std::ios::app);
  else if(flow==2)
  scheduler_read.open("FreshScenarios/Scheduler2.txt", std::ios::app);
  else if(flow==3)
  scheduler_read.open("FreshScenarios/Scheduler3.txt", std::ios::app);
  else if(flow==4)
  scheduler_read.open("FreshScenarios/Scheduler4.txt", std::ios::app);
  else if(flow==5)
  scheduler_read.open("FreshScenarios/Scheduler5.txt", std::ios::app);
  else if(flow==6)
  scheduler_read.open("FreshScenarios/Scheduler6.txt", std::ios::app);
  else if(flow==7)
  scheduler_read.open("FreshScenarios/Scheduler7.txt", std::ios::app);
  else if(flow==8)
  scheduler_read.open("FreshScenarios/Scheduler8.txt", std::ios::app);
  else if(flow==9)
  scheduler_read.open("FreshScenarios/Scheduler9.txt", std::ios::app);
  else if(flow==10)
  scheduler_read.open("FreshScenarios/Scheduler10.txt", std::ios::app);
  else if(flow==11)
  scheduler_read.open("FreshScenarios/Scheduler11.txt", std::ios::app);

//simulation parameters
 float alpha=0.80;
  
  
  double STA1_CurrentDelay_link1=0,STA1_CurrentDelay_link2=0,STA1_CurrentDelay=0;

  if(Simulator::Now ().GetMilliSeconds()>2500)
  {
    num_pktNoSLA=0;
    num_pktSLA=0;
    read(STA1_CurrentDelay_link1,STA1_CurrentDelay_link2,STA1_CurrentDelay,flow,D);//reading current delay from file
      SLA_ViolationError= (num_pktNoSLA*1.0)/( num_pktNoSLA+ num_pktSLA);
     
  }

  
 int temp_count_l1=0,temp_count_l2=0;


  

  if(SLA_ViolationError<=T){
          ...
          //   if(Avg_pktDelay_STA1_link1>Avg_pktDelay_STA1_link2)
          //   {
          //     PbSTA1_link1=1;
          //     PbSTA1_link2=0;
          ...
          //   }
          //   else if(Avg_pktDelay_STA1_link2>Avg_pktDelay_STA1_link1){
          //     PbSTA1_link1=0;
          //     PbSTA1_link2=1;
          ...
          //  if (WifiMac::flag_Ext_CSLAMLO!=-1) //change 3

          //   {
       
              
          //     if(WifiMac::flag_Ext_CSLAMLO==1) // if link 1 is best and if this STA is on link 1 then change probability
          //     {
          //       PbSTA1_link1=0;
          //       PbSTA1_link2=1;
          
          //     }
          //     if(WifiMac::flag_Ext_CSLAMLO==2 ) // if link 1 is best and if this STA is on link 1Wi then change probability
          //     {
          //       PbSTA1_link1=1;
          //       PbSTA1_link2=0;
          
          //     }
          //   //  WifiMac::flag_Ext_CSLAMLO=-1;
          //   }
          //   else
          //   {
          //      PbSTA1_link1=0.5;
          // PbSTA1_link2=0.5;
          // FlagArray[flow-1]=false;// interflow code
          //   }
          // interflow code
  ...
  {   
    ...
    FlagArray[flow-1]=true;// interflow code
        if(Avg_pktDelay_STA1_link1>Avg_pktDelay_STA1_link2)
     {
        WifiMac::flag_Ext_CSLAMLO=2;
     ...
     Avg_pktDelay_STA1=alpha*(Avg_pktDelay_STA1)+STA1_CurrentDelay*(1-alpha);

     if(Avg_pktDelay_STA1_link1>=D)
     {
      PbSTA1_link1=0;
      PbSTA1_link2=1;

     }
     if(Avg_pktDelay_STA1_link2>=D)
     {
      PbSTA1_link1=1;
      PbSTA1_link2=0;

     }
     if((Avg_pktDelay_STA1_link1<D&&Avg_pktDelay_STA1_link2<D)||(Avg_pktDelay_STA1_link1>D&&Avg_pktDelay_STA1_link2>D))
     {
      double temp_link1=(1/(Avg_pktDelay_STA1_link1==0?2:(Avg_pktDelay_STA1_link1)));
      double temp_link2=(1/(Avg_pktDelay_STA1_link2==0?2:(Avg_pktDelay_STA1_link2)));
      PbSTA1_link1=temp_link1/(temp_link1+temp_link2);


      PbSTA1_link2=1-PbSTA1_link1;
      // if(Avg_pktDelay_STA1_link1==0&&Avg_pktDelay_STA1_link2==0)
  ...
  if(PbSTA1_link1==0&&PbSTA1_link2==0)
   SLA_ViolationError=0;
scheduler_read<<now<<","<<PbSTA1_link1<<","<<PbSTA1_link2<<","<<SLA_ViolationError<<std::endl;
 
  scheduler_read.close();
  

}//scheduler ends

void WifiMac::read(double &STA1_CurrentDelay_link1,double &STA1_CurrentDelay_link2,double &STA1_CurrentDelay,int flow, float D){
  float alpha=0.80;
    Avg_pktDelay_STA1_link1=0;
    Avg_pktDelay_STA1_link2=0;

  
  std::ifstream read_link1,read_link2,read_STA1;
  if(flow==1)
  {
  read_link1.open("FreshScenarios/LogDelaySTA1Link1.txt");
  read_link2.open("FreshScenarios/LogDelaySTA1Link2.txt");
  read_STA1.open("FreshScenarios/LogDelayThroughputSTA1.txt");

  }
  else if(flow ==2)
  {
    read_link1.open("FreshScenarios/LogDelaySTA2Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA2Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA2.txt");
    
    // read_link1.open("FreshScenarios/LogDelaySTA2Link1_flow1.txt");
    // read_link2.open("FreshScenarios/LogDelaySTA2Link2_flow1.txt");
    // read_STA1.open("FreshScenarios/LogDelayThroughputSTA2_flow1.txt");
  }
  else if(flow ==3)
  {
     read_link1.open("FreshScenarios/LogDelaySTA3Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA3Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA3.txt");
    // read_link1.open("FreshScenarios/LogDelaySTA2Link1_flow2.txt");
    // read_link2.open("FreshScenarios/LogDelaySTA2Link2.txt_flow2");
    // read_STA1.open("FreshScenarios/LogDelayThroughputSTA2_flow2.txt");
  }
  else if(flow ==4)
  {
     read_link1.open("FreshScenarios/LogDelaySTA4Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA4Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA4.txt");
  }
  else if(flow ==5)
  {
     read_link1.open("FreshScenarios/LogDelaySTA5Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA5Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA5.txt");
  }
  else if(flow ==6)
  {
     read_link1.open("FreshScenarios/LogDelaySTA6Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA6Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA6.txt");
  }
  else if(flow ==7)
  {
     read_link1.open("FreshScenarios/LogDelaySTA7Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA7Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA7.txt");
  }
  else if(flow ==8)
  {
     read_link1.open("FreshScenarios/LogDelaySTA8Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA8Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA8.txt");
  }
  else if(flow ==9)
  {
     read_link1.open("FreshScenarios/LogDelaySTA9Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA9Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA9.txt");
  }
  else if(flow ==10)
  {
     read_link1.open("FreshScenarios/LogDelaySTA10Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA10Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA10.txt");
  }
  else if(flow ==11)
  {
     read_link1.open("FreshScenarios/LogDelaySTA11Link1.txt");
    read_link2.open("FreshScenarios/LogDelaySTA11Link2.txt");
    read_STA1.open("FreshScenarios/LogDelayThroughputSTA11.txt");
  }
  //reading delay for STA1
  if(read_link1){
    while(!read_link1.eof())
    { 
      
       double a,time_;
      char b;
      
      read_link1>>time_>>b>>a>>b>>STA1_CurrentDelay_link1>>b>>a>>b>>a;

      if(time_>=2500000)
        Avg_pktDelay_STA1_link1=alpha*(Avg_pktDelay_STA1_link1)+STA1_CurrentDelay_link1*(1-alpha);
     }
  }
  read_link1.close();

  if(read_link2){
    while(!read_link2.eof()) 
    {
     
      double a,time_;
      char b;
      read_link2>>time_>>b>>a>>b>>STA1_CurrentDelay_link2>>b>>a>>b>>a;
       
      if(time_>=2500000)
       Avg_pktDelay_STA1_link2=alpha*(Avg_pktDelay_STA1_link2)+STA1_CurrentDelay_link2*(1-alpha);
  ...
  if(read_STA1){
    while(!read_STA1.eof()) {
      ...
      read_STA1>>time>>b>>a>>b>>STA1_CurrentDelay>>b>>a;
      
      if(time>=2500000){
        if(STA1_CurrentDelay>D)
          num_pktNoSLA++;
        else
          num_pktSLA++;


         double diff=time-TT_current;
          if(diff>=.5*1000000)
          {
              TT_current=time;        
        
            num_pktNoSLA=0.5*num_pktNoSLA;
              num_pktSLA=0.5*num_pktSLA;
...
WifiMac::NotifyChannelSwitching (void)
{
  ...
  // in a different band
  GetLink (0).stationManager->SetupPhy (GetLink (0).phy);
...
WifiMac::ConfigurePhyDependentParameters (void)
{
  ...
  WifiStandard standard = GetLink (0).phy->GetStandard ();
...
Ptr<WifiPhy> WifiMac::GetWifiPhy (uint8_t linkId) const
{
...
WifiMac::Receive (Ptr<WifiMacQueueItem> mpdu, uint8_t linkId)
{
  ...
  auto& link = GetLink (linkId);//  auto& link = GetLink (Singl_link_op_Id);
