...
void WifiHelper::fun() const{
  std::cout<<"hi";
}
NetDeviceContainer
WifiHelper::Install (const WifiPhyHelper &phyHelper1,const WifiPhyHelper &phyHelper2,
  ...
  for (NodeContainer::Iterator i = first; i != last; ++i)
    {
      ...
      std::vector<Ptr<WifiPhy>> phys1 = phyHelper1.Create (node, device);
      std::vector<Ptr<WifiPhy>> phys2 = phyHelper2.Create (node, device);
         std::vector<Ptr<WifiPhy>> phys;
       phys.push_back(phys1.at(0));
       phys.push_back(phys2.at(0));
       
      //std::cout<<"Wifi-Helperclass "<<phys.size()<<"\n";
     device->SetPhy (phys.at(0));
    device->SetPhy (phys.at(1));
...
WifiHelper::Install (const WifiPhyHelper &phyHelper1,const WifiPhyHelper &phyHelper2,
                     const WifiMacHelper &macHelper, NodeContainer c) const
{
  return Install (phyHelper1,phyHelper2, macHelper, c.Begin (), c.End ());
}
...
//custom code 2
NetDeviceContainer
WifiHelper::Install (const WifiPhyHelper &phy1,const WifiPhyHelper &phy2,
                     const WifiMacHelper &mac, std::string nodeName) const
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  return Install (phy1,phy2, mac, NodeContainer (node));
}
